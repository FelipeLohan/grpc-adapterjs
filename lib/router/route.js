...
/**
 * Attach the request/response codecs `app.service()` resolved from the
 * `.proto` (plan §7.2). `call.js` expects `requestType`/`responseType` to
 * expose `decodeMessage(payload, app)`/`encodeMessage(message, app)` —
 * see `lib/codec/protobuf.js#createCodec`.
 *
 * @param {{decodeMessage: Function}} requestType
 * @param {{encodeMessage: Function}} responseType
 * @return {Route} for chaining
 * @public
 */

Route.prototype.codec = function codec (requestType, responseType) {
...
Route.prototype.dispatch = function dispatch (call, done) {
  call.type = this.type
  call.requestType = this.requestType
  call.responseType = this.responseType

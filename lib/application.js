...
app.defaultConfiguration = function defaultConfiguration () {
  ...
  // plan §7.2: verify() before encode() by default; app.disable('strict
  // encode') opts out. toObject() defaults matter most for int64 (String,
  // avoiding precision loss) and bytes (Buffer).
  this.enable('strict encode')
  this.set('protobuf toObject', codecs.DEFAULT_TO_OBJECT)
...
/**
 * Register every rpc of a `.proto` service in one call (plan §7.1),
 * deriving path, codec and call type from its reflection instead of
 * registering them by hand through `app.unary`/`app.serverStream`/etc.
 *
...
app.service = function service (serviceOrPath, impl) {
  return registerService.call(this, serviceOrPath, impl)
}

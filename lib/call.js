...
call[Symbol.asyncIterator] = function asyncIterator () {
  ...
  return {
    ...
    return: function (value) {
      // stopping early (a `break`, a handler returning without reading
      // everything) must drain whatever request bytes are still unread
      // *before* the decoder gets torn down below -- see drainRequest().
      self.drainRequest()
...
/**
 * Discard any request bytes the handler never read. A clientStream/bidi
 * handler is free to stop consuming `call` early (`break`, an early
 * `return`, an error) without reading the peer's remaining messages —
 * without this, the unread bytes would sit in `this.stream`'s internal
 * buffer forever, the stream would never see its own `'end'`, and closing
 * the server would hang waiting for a "connection" that's actually just
 * abandoned request data.
 *
 * Idempotent (tracked by `_requestDrained`, not by the decoder's own
 * `destroyed` flag): the async iterator's `return()`/`throw()` already call
 * this the moment a `break`/early-return/thrown-error tears it down, which
 * happens *before* `call.end()`/`fail()` get a chance to run — checking
 * `this._decoder.destroyed` there would already be true and wrongly skip
 * draining `this.stream` itself.
 *
 * @private
 */

call.drainRequest = function drainRequest () {
  if (this._requestDrained) return

  this._requestDrained = true

  if (this._decoder && !this._decoder.destroyed) {
    this.stream.unpipe(this._decoder)
    this._decoder.destroy()
  }

  if (!this.stream.destroyed && this.stream.readable) this.stream.resume()
}
